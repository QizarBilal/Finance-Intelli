...
import { ArrowRightLeft, Building2, CreditCard, Landmark, Plus, RefreshCw, WalletCards } from 'lucide-react';
...
export default function Accounts() {
  ...
  return (
    <div className="space-y-7 pb-10">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{rows.map(account => { const Icon = account.type === 'bank' ? Landmark : account.type === 'credit_card' ? CreditCard : Building2; return <article key={account.id} className="glass-card-hover rounded-2xl p-5"><div className="flex justify-between"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary/10 text-primary"><Icon className="h-5 w-5" /></div><span className="text-xs capitalize text-muted-foreground">{account.type.replace('_',' ')}</span></div><p className="mt-5 text-sm text-muted-foreground">{account.institution || account.currency}</p><h2 className="truncate text-lg font-semibold">{account.name}</h2><p className="mt-3 font-mono text-2xl font-bold">{formatCurrency(account.currentBalance)}</p><p className="mt-4 border-t pt-3 text-xs text-muted-foreground">Opening {formatCurrency(account.openingBalance)}</p></article>; })}</div>}
    </div>
  );
}
